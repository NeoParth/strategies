...
def get_position(symbol, account):
    ...
    print(f'this is current account value: {user_state["marginSummary"]["accoountValue"]}')
    positions = []
...
def get_position_andmaxpos(symbol, account):
    '''
    gets the current position info, like size etc.
    '''
    # account = LocalAccount = eth_account.Account.from_key(key)
    ...
    for position in user_state["assetPositions"]:
        ...
        else: print(f'we are in {len(open_positions)} positions and max pos is {max_positions}...not closing positions')

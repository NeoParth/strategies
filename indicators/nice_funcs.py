...
def df_rsi():
    ...
    df_rsi['timestamp'] = pd.to_datetime(df_sma['timestamp'], unit='ms')
...
# VWAP : Volume Weighted Average Price - intraday indicator: sum(price * volume) from market open until t
# Beating the VWAP - trading better than the other participants in the market
def get_df_vwap():

    ### GETTING NEWEST DATAFRAME ###
    bars = bitget.fetch_ohlcv(symbol, timeframe=timeframe, limit=limit)
    df_vwap = pd.DataFrame(bars, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    df_vwap['timestamp'] = pd.to_datetime(df_sma['timestamp'], unit='ms')
...
def vwap_indi():
    # bring in the df and do the following math formula
    print('starting the vwma indicator to see if to be long or short')

    df_vwap = get_df_vwap()
    df_vwap['colXclose'] = df_vwap['close']*df_vwap['volume']
    # sum previous 13 values of volXclose - here experiment with other values to see how it changes the indicator

    # get cummulative sum of vol here, which is the last
    df_vwap['cum_vol'] = df_vwap['volume'].cumsum()

    # then cum sum of vol * sum
    df_vwap['cum_volXclose'] = (df_vwap['volume']) * (df_vwap['cum_vol'])
    df_vwap = df_vwap.fillna(0)

    # vwap = sum(volume * avg price) / sum(vol)
